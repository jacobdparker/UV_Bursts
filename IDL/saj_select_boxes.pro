...
;
;PURPOSE:
;  VERSION OF SELECT_BOXES THAT USES THE SLIDE_IMAGE WIDGET TO HANDLE REALLY BIG IMAGES!!!
...
pro show_image
  ...
  tv, bytscl(img.image, img.i0, img.i1)
  ...
  CASE action OF
     ...
     'DISPLAY' : begin
        ...
        if mouseread.down eq 1 then begin
           ...
           ;plot box
           plot, [mouseread.x0[i], event.x, event.x, $
                  mouseread.x0[i], mouseread.x0[i]], $
                 [mouseread.y0[i], mouseread.y0[i], $
                  event.y, event.y, mouseread.y0[i]],$
        ...
        if event.release eq 1 then begin
           ...
           mouseread.x1[i]=max([mouseread.x0[i],event.x])
           mouseread.x0[i]=min([mouseread.x0[i],event.x])

           mouseread.y1[i]=max([mouseread.y0[i],event.y])
           mouseread.y0[i]=min([mouseread.y0[i],event.y])
...
pro display_widget_setup
  ...
  ss=get_screen_size()
  if img.nx gt ss[0]*0.9 or img.ny gt ss[1]*0.9 then begin
     drawwidget = widget_draw(basewidget, uvalue='DISPLAY', $
                              xsize=img.nx, ysize=img.ny, /SCROLL, $
                              x_scroll_size=ss[0]*0.6, y_scroll_size=ss[1]*0.6,$
