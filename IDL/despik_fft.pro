...
;  2017-4-29 J. Parker grabbed CCK's despik routine and
;  iplemented the idl function convol_fft for a speed up

function despik_fft,data, sigmas=sigmas, Niter=Niter, kernel=kernel, min_std=min_std, $
...
for i=1, Niter do begin
   neighborhood_mean = nconvol_fft(goodmap*data, kernel) / convol_fft(goodmap,kernel)
   case mode of
   ...
   endcase
   neighborhood_std = sqrt( convol_fft(goodmap*deviation^2, kernel) / convol_fft(goodmap,  kernel) ) > min_std
...
neighborhood_mean = convol_fft(goodmap*data, k2) / $
                    convol_fft(goodmap, k2)
